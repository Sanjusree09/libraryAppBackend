...
app.put('/updateBooks/:id', async (req: Request, res: Response, next: NextFunction):Promise<void> => {
  ...
  app.post('/admin/Signup',  (req: Request, res: Response, next: NextFunction)=> {
    ...
    try {
       ...
       AppDataSource.manager.save(adminRegister);
       res.status(201).json({
...
app.put('/updateMember/:id', async (req: Request, res: Response, next: NextFunction):Promise<void> => {
  ...
  if (!memberName || !memberEmail || !memberContactNumber || !memberAddress || !memberOccupation ) {
     res.status(400).json({ message: 'Missing required fields' });
  }
  ...
  try {
    ...
    member.memberContactNumber = memberContactNumber;
    member.memberAddress = memberAddress;
    member.memberOccupation = memberOccupation;
